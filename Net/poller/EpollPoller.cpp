...
void EpollPoller::remove_channel(int fd) {
    ...
    operate(EPOLL_CTL_DEL, sender->second);
...
void EpollPoller::update_channel(Channel *channel) {
    ...
    auto sender = _channels.find(fd);
    assert(sender != _channels.end());
    if (sender->second == nullptr) {
        operate(EPOLL_CTL_ADD, channel);
        sender->second = channel;
...
void EpollPoller::get_events(Poller::ChannelList &list, int size) {
    ...
    for (int i = 0; i < size; ++i) {
        auto sender = _channels.find(_eventsQueue[i].data.fd);
        assert(sender != _channels.end());
        sender->second->set_revents((short) _eventsQueue[i].events | sender->second->revents());
        list.push_back(sender->second);
    }
}
